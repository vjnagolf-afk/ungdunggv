...
def export_to_docx_vietnam_standard(text_content, title_name, school_name="TRƯỜNG THCS NGUYỄN CHÍ THANH", group_name="TỔ KHOA HỌC TỰ NHIÊN - GDTC"):
    ...
    doc.save(bio)
    return bio.getvalue()
# 🌟 ĐỒNG BỘ: Sửa định nghĩa hàm chỉ nhận đúng 1 tham số để dập tắt hoàn toàn lỗi TypeError ở app.py
def render_exam_designer_section(run_ai_prompt_safe_func=None):
    ...
    /* Thiết kế thanh tiêu đề màu hồng và xanh song hành ngang hàng mượt mà */
    ...
    /* Cấu hình các khối tổng số câu và tổng điểm đồng bộ hàng kẻ nằm ngang dưới tiêu đề */
    ...
    with tab_thiet_ke:
        ...
        # 🌟 KHỞI TẠO KHUNG ĐỎ ÔM TRỌN: Đặt toàn bộ tiêu đề và lưới ô vào trong khối div này
        ...
        with col_main1:
            ...
            # Khối tổng câu và tổng điểm xếp ngay ngắn dưới tiêu đề hồng
            st.markdown(
            ...
            # Ô nhập liệu thành phần trắc nghiệm
        ...
        with col_main2:
            ...
            # Khối tổng số câu tự luận nằm ngang hàng mượt mà dưới tiêu đề xanh
            st.markdown(
        ...
        st.markdown("</div>", unsafe_allow_html=True) # ĐÓNG KHUNG ĐỎ (Khung đỏ bao trọn vẹn toàn bộ 2 cột dữ liệu)
        st.markdown("<br>", unsafe_allow_html=True)
        # --- CẤU HÌNH HÀNG MỨC ĐỘ NHẬN THỨC ---
        ...
        if run_exam_ai:
            ...
            prompt_exam = (
                ...
                f"- Tự luận tổng cộng {num_tl_input} câu lớn with tổng điểm là {tong_diem_tl_real} điểm.\n"
