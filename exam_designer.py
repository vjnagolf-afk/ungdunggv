...
def export_to_docx_vietnam_standard(text_content, title_name, school_name="TRƯỜNG THCS NGUYỄN CHÍ THANH", group_name="TỔ KHOA HỌC TỰ NHIÊN - GDTC"):
    ...
    # HÀM XỬ LÝ TEXT THÔNG MINH: NHẬN DIỆN IN ĐẬM VÀ CHỈ SỐ HOÁ HỌC/TOÁN HỌC
    def process_runs(paragraph, text):
        bold_parts = text.split('**')
        for i, b_part in enumerate(bold_parts):
            is_bold = (i % 2 != 0)
            # Tách các thẻ <sub> và <sup> ra khỏi chuỗi
            sub_sup_parts = re.split(r'(<sub>.*?</sub>|<sup>.*?</sup>)', b_part)
            for part in sub_sup_parts:
                if not part: continue
                if part.startswith('<sub>') and part.endswith('</sub>'):
                    run = paragraph.add_run(part[5:-6]) # Bỏ tag <sub>...</sub>
                    run.bold = is_bold
                    run.font.subscript = True # Đẩy xuống thành chỉ số dưới (Hóa học)
                elif part.startswith('<sup>') and part.endswith('</sup>'):
                    run = paragraph.add_run(part[5:-6]) # Bỏ tag <sup>...</sup>
                    run.bold = is_bold
                    run.font.superscript = True # Đẩy lên thành chỉ số trên (Toán học)
                else:
        ...
        for r_idx, row in enumerate(table_data):
            for c_idx, cell_val in enumerate(row):
                if c_idx < cols:
                    ...
                    p = cell.paragraphs[0]
                    p.text = "" # Xóa text mặc định của ô
...
PHẦN 3. ĐỀ KIỂM TRA VÀ ĐÁP ÁN CHI TIẾT (Chia rõ Trắc nghiệm và Tự luận)"""
                        ...
                        st.markdown(result_text, unsafe_allow_html=True) # Cho phép render HTML trên Streamlit
