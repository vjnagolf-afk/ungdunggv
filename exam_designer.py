# exam_designer.py - BẢN HOÀN THIỆN 100% GIAO DIỆN VÀ TÍNH NĂNG
...
# ================= ĐOẠN 2: GIAO DIỆN CHÍNH (ÉP CSS THEO BẢN PHÁC HỌA) =================
def render_exam_designer_section(run_ai_prompt_safe_func):
    
    # CSS Customization for Tabs and Inputs to match the image exactly
    st.markdown("""
        <style>
    ...
    with tab_tao_de:
        ...
        with col1:
            st.markdown("<h5 style='color:blue; font-weight:bold; text-align:center;'>MENU MÔN HỌC</h5>", unsafe_allow_html=True)
            st.session_state["save_mon_hoc"] = st.text_input("Môn học:", value=st.session_state["save_mon_hoc"], label_visibility="collapsed")
        with col2:
            st.markdown("<h5 style='color:blue; font-weight:bold; text-align:center;'>MENU LỚP</h5>", unsafe_allow_html=True)
        ...
        with col3:
            st.markdown("<h5 style='color:blue; font-weight:bold; text-align:center;'>HÌNH THỨC KT</h5>", unsafe_allow_html=True)
            hinh_thuc = st.selectbox("Hình thức đề:", ["Trắc nghiệm kết hợp tự luận", "100% Trắc nghiệm", "100% Tự luận"], label_visibility="collapsed")
        with col4:
            st.markdown("<h5 style='color:blue; font-weight:bold; text-align:center;'>THỜI GIAN</h5>", unsafe_allow_html=True)
        ...
        st.markdown("<h5 style='color:red; font-weight:bold; margin-top:20px;'>Tỷ lệ mức độ nhận thức (%):</h5>", unsafe_allow_html=True)
        col_nb, col_th, col_vd, col_vdc = st.columns(4)
        with col_nb:
            c_l, c_r = st.columns([1, 1])
            c_l.markdown("<p style='font-weight:bold; font-size:18px; text-align:right; margin-top:5px;'>Nhận biết:</p>", unsafe_allow_html=True)
            tl_nhan_biet = c_r.number_input("nb", min_value=0, max_value=100, value=40, step=10, label_visibility="collapsed")
        with col_th:
            c_l, c_r = st.columns([1, 1])
            c_l.markdown("<p style='font-weight:bold; font-size:18px; text-align:right; margin-top:5px;'>Thông hiểu:</p>", unsafe_allow_html=True)
            tl_thong_hieu = c_r.number_input("th", min_value=0, max_value=100, value=30, step=10, label_visibility="collapsed")
        with col_vd:
            c_l, c_r = st.columns([1, 1])
            c_l.markdown("<p style='font-weight:bold; font-size:18px; text-align:right; margin-top:5px;'>Vận dụng:</p>", unsafe_allow_html=True)
            tl_van_dung = c_r.number_input("vd", min_value=0, max_value=100, value=20, step=10, label_visibility="collapsed")
        with col_vdc:
            c_l, c_r = st.columns([1.2, 1])
            c_l.markdown("<p style='font-weight:bold; font-size:18px; text-align:right; margin-top:5px;'>Vận dụng cao:</p>", unsafe_allow_html=True)
            tl_vd_cao = c_r.number_input("vdc", min_value=0, max_value=100, value=10, step=10, label_visibility="collapsed")

        st.markdown("<br><br>", unsafe_allow_html=True)

        # --- HÀNG 3: TÊN ĐỀ & UPLOAD FILE CHỮ ĐỎ NGHIÊNG ---
        col_name, col_f1, col_f2 = st.columns([2, 1.2, 1.2])
        with col_name:
            st.markdown("<h5 style='color:red; font-weight:bold;'>Tên bài kiểm tra / Đề số:</h5>", unsafe_allow_html=True)
            st.session_state["save_ten_de"] = st.text_input("tende", value=st.session_state["save_ten_de"], label_visibility="collapsed")
            st.markdown("<p style='font-style:italic; font-size:18px; text-align:center;'>Ví dụ: Kiểm tra đánh giá giữa kì I</p>", unsafe_allow_html=True)
        with col_f1:
            st.markdown("<p style='color:red; font-style:italic; font-weight:bold; text-align:center;'>Tải Đề Cương (.docx, .pdf):</p>", unsafe_allow_html=True)
            file_de_cuong = st.file_uploader("f1", type=["docx", "pdf"], key="exam_upload_de_cuong_2026", label_visibility="collapsed")
        with col_f2:
            st.markdown("<p style='color:red; font-style:italic; font-weight:bold; text-align:center;'>Tải Đề mẫu ma trận (.docx, .pdf):</p>", unsafe_allow_html=True)
        ...
        # --- HÀNG 4: BỐ CỤC TRẮC NGHIỆM VÀ TỰ LUẬN ĐỒNG BỘ MÀU ---
        col_tn, col_space, col_tl = st.columns([1, 0.1, 1])
        ...
        with col_tn:
            ...
            c1.markdown("<p style='font-size:16px; font-weight:bold; margin-top:10px;'>Số câu nhiều lựa chọn:</p>", unsafe_allow_html=True)
            ...
            c4.markdown("<p style='font-style:italic; font-size:16px; margin-top:10px;'>điểm</p>", unsafe_allow_html=True)
            
            c1, c2, c3, c4 = st.columns([2.5, 1, 1, 0.8])
            c1.markdown("<p style='font-size:16px; font-weight:bold; margin-top:10px;'>Số câu đúng/sai:</p>", unsafe_allow_html=True)
            ...
            c1, c2, c3, c4 = st.columns([2.5, 1, 1, 0.8])
            c1.markdown("<p style='font-size:16px; font-weight:bold; margin-top:10px;'>Số câu điền khuyết:</p>", unsafe_allow_html=True)
            ...
            c1, c2, c3, c4 = st.columns([2.5, 1, 1, 0.8])
            c1.markdown("<p style='font-size:16px; font-weight:bold; margin-top:10px;'>Số câu trả lời ngắn:</p>", unsafe_allow_html=True)
            ...
            # CẬP NHẬT HEADER TRẮC NGHIỆM ĐÃ TÍNH TỔNG (Khoá)
            header_tn_placeholder.markdown(f"""
            <div style="background-color: #FFF2CC; padding: 12px; display: flex; align-items: center; justify-content: center; gap: 20px;">
                <span style="color: blue; font-size: 20px; font-weight: bold;">TRẮC NGHIỆM</span>
                <span style="background-color: white; border: 1px solid #FFD700; color: blue; padding: 5px 20px; font-weight: bold; font-size: 20px;">{tong_diem_tn:.1f}</span>
                <span style="color: blue; font-size: 20px; font-weight: bold;">Điểm</span>
            </div><br>
            """, unsafe_allow_html=True)

        with col_tl:
            # HEADER TL: Yêu cầu ô nhập câu nằm ngay trên Header
            h1, h2, h3, h4 = st.columns([1.5, 0.8, 1, 1])
            with h1: st.markdown("<div style='background-color:#E2EEDB; height:50px; line-height:50px; text-align:center; color:blue; font-weight:bold; font-size:20px;'>TỰ LUẬN</div>", unsafe_allow_html=True)
            ...
            with h4: st.markdown("<div style='background-color:#E2EEDB; height:50px; line-height:50px; text-align:center; color:blue; font-weight:bold; font-size:20px;'>Điểm</div>", unsafe_allow_html=True)
            ...
            for i in range(int(sc_tu_luan)):
                ...
                c1.markdown(f"<p style='font-size:16px; font-weight:bold; text-align:center; margin-top:10px;'>Câu {i+1}.</p>", unsafe_allow_html=True)
                d_c = c2.number_input(f"tl_{i}", min_value=0.0, value=2.5 if i==0 else (1.5 if i==1 else 1.0), step=0.25, label_visibility="collapsed")
                c3.markdown("<p style='font-style:italic; font-size:16px; margin-top:10px;'>điểm</p>", unsafe_allow_html=True)
                tong_diem_tl += d_c
                diem_chi_tiet_tl.append((i+1, d_c))
                
            # CẬP NHẬT HEADER TỰ LUẬN (Khoá số màu đỏ)
            ph_tong_diem_tl.markdown(f"<div style='background-color:white; border:2px solid #FFD700; height:50px; line-height:45px; text-align:center; color:red; font-weight:bold; font-size:20px;'>{tong_diem_tl:.1f}</div>", unsafe_allow_html=True)

        # --- HÀNG 5: YÊU CẦU KHÁC ---
        st.markdown("<br><p style='color:red; font-weight:bold; text-decoration:underline; font-style:italic; margin-bottom:5px;'>Yêu cầu khác:</p>", unsafe_allow_html=True)
