...
# --- 🌟 HÀM MỚI: TRÍCH XUẤT CHỮ TỪ FILE TÀI LIỆU ĐỂ LÀM NỀN NGỮ CẢNH ---
def extract_text_from_file(uploaded_file):
    ...
    try:
        if file_name.endswith(".txt"):
            ...
            # Để tránh cài đặt thư viện nặng khi chấm thi, đọc chuỗi byte thô hoặc text thô từ luồng
...
def render_personal_plan(run_ai_handler=None):
    ...
    with st.form("form_personal_plan_fixed_final_v8", border=False):
        ...
        # 🌟 WIDGET MỚI THÊM: Tải tệp tài liệu SGK nền
    ...
    if run_ai_plan:
        if not t_name or not grade_target:
            ...
            with st.spinner("Trợ lý AI đang đọc tệp tài liệu và phân bổ dữ liệu Phụ lục III..."):
                # Trích xuất nội dung từ tệp SGK được tải lên
                context_document = ""
                if sgk_file is not None:
                    context_document = extract_text_from_file(sgk_file)
                
                # Đưa ngữ cảnh tài liệu tham khảo vào Prompt để ép mô hình bám sát dữ liệu
                prompt_plan = (
                    ...
                    f"Đặc biệt, tại cột 'YeuCauCanDat', bạn phải trích xuất, phân tích và bám sát chính xác theo tài liệu tham khảo được cung cấp sau đây:\n"
                    f"[DỮ LIỆU THAM KHẢO TỪ FILE TẢI LÊN]:\n{context_document}\n\n"
                    f"Mỗi bài học lớn phải được tách nhỏ thành các hàng đơn lẻ tương ứng với từng 'Tiết CT' (Ví dụ: Bài 1 (Tiết 1) xếp ở 1 hàng riêng, Bài 1 (Tiết 2) xếp ở hàng tiếp theo). "
                ...
                if run_ai_handler is not None:
                    ...
                    if status == "error":
                        ...
                        try:
                            ...
                            st.success("🎉 Khởi tạo và đồng bộ kế hoạch dựa trên tài liệu tham khảo thành công!")
